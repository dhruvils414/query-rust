    ...
    fn rewrite(
        ...
        let new_plan = match child_plan {
            LogicalPlan::Filter(ref child_filter) => {
                ...
                let new_filter = LogicalPlan::Filter(Filter::try_new_with_op(
                    new_predicate,
                    child_filter.input.clone(),
                    child_filter.filter_op
                )?);
            ...
            LogicalPlan::Union(ref union) => {
                ...
                for input in &union.inputs {
                    ...
                    let push_predicate =
                        replace_cols_by_name(filter.predicate.clone(), &replace_map)?;
                    inputs.push(Arc::new(LogicalPlan::Filter(Filter::try_new_with_op(
                        push_predicate,
                        input.clone(),
                        filter.filter_op
                    )?)))
            ...
            LogicalPlan::Aggregate(ref agg) => {
                ...
                let child = match conjunction(replaced_push_predicates) {
                    Some(predicate) => LogicalPlan::Filter(Filter::try_new_with_op(
                        predicate,
                        agg.input.clone(),
                        filter.filter_op
                    )?),
            ...
            LogicalPlan::Extension(ref extension_plan) => {
                ...
                let new_children = match conjunction(push_predicates) {
                    Some(predicate) => extension_plan
                        ...
                        .into_iter()
                        .map(|child| {
                            Ok(LogicalPlan::Filter(Filter::try_new_with_op(
                                predicate.clone(),
                                Arc::new(child.clone()),
                                filter.filter_op
                            )?))
...
fn convert_to_cross_join_if_beneficial(plan: LogicalPlan) -> Result<LogicalPlan> {
    if let LogicalPlan::Join(join) = &plan {
        ...
        return Filter::try_new_with_op(filter.predicate.clone(), Arc::new(new_input), filter.filter_op)
